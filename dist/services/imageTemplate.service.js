...
const TRACKER_FILE = path_1.default.join(process.cwd(), "template_tracker.json");
const TEMPLATE_DIR = path_1.default.join(process.cwd(), "public", "templates");
/**
 * Returns a random quote from your local database.
 * No external API calls = 100% reliability.
 */
async function getRandomQuote() {
    try {
        const randomIndex = Math.floor(Math.random() * quotes_1.motivationalQuotes.length);
        let quote = quotes_1.motivationalQuotes[randomIndex];
        // Clean up any existing quotes to avoid double-quotes
        const cleanQuote = quote.replace(/^["']+|["']+$/g, '');
        return `"${cleanQuote}"`;
    }
    catch (error) {
        console.error("❌ Error picking local quote:", error);
        return `"The only way to do great work is to love what you do."`;
    }
}
/**
 * Grabs a background image.
 * High-speed, no-logo, and toggles between local and cloud.

/**
 * High-speed background selector with 40/60 probability split.
 * Automatically falls back to local folder if website fails.
 */
/**
 * 1. GET UNIQUE IMAGE (The Memory Logic)
 * Ensures no repeat within a 10-post cycle.
 */
function getNextUniqueImage() {
    const allImages = fs_1.default.readdirSync(TEMPLATE_DIR).filter(file => ['.jpg', '.jpeg', '.png'].includes(path_1.default.extname(file).toLowerCase()));
    let tracker = { used: [] };
    if (fs_1.default.existsSync(TRACKER_FILE)) {
        tracker = JSON.parse(fs_1.default.readFileSync(TRACKER_FILE, "utf-8"));
    }
    let available = allImages.filter(img => !tracker.used.includes(img));
    // Reset if no unique images left
    if (available.length === 0) {
        tracker.used = [];
        available = allImages;
    }
    const selected = available[Math.floor(Math.random() * available.length)];
    tracker.used.push(selected);
    if (tracker.used.length > 10)
        tracker.used.shift(); // Keep memory to last 10
    fs_1.default.writeFileSync(TRACKER_FILE, JSON.stringify(tracker, null, 2));
    return selected;
}
/**
 * 2. FETCH BACKGROUND (The Probability Logic)
 * 40% Local / 60% Web with instant fallback.
 */
async function fetchBackground() {
    const useLocal = Math.random() < 0.4;
    if (useLocal && fs_1.default.existsSync(TEMPLATE_DIR)) {
        try {
            const imageName = getNextUniqueImage();
            return await (0, canvas_1.loadImage)(path_1.default.join(TEMPLATE_DIR, imageName));
        }
        catch (e) {
            console.log("Local fetch failed, switching to web...");
        }
    }
    try {
        // Using Picsum for speed and reliability (No logo)
        return await (0, canvas_1.loadImage)(`https://picsum.photos/1080/1080?random=${Date.now()}`);
    }
    catch (error) {
        console.error("🌐 Web API failed. Using emergency local fallback...");
        const files = fs_1.default.readdirSync(TEMPLATE_DIR);
        return await (0, canvas_1.loadImage)(path_1.default.join(TEMPLATE_DIR, files[0]));
    }
}
/**
 * 3. MAIN GENERATION (The Professional Layout)
 */
async function createImageFromTemplate(text) {
    const width = 1080;
    const height = 1080;
    ...
    // Subtle dark overlay to add depth
    ctx.fillStyle = "rgba(0, 0, 0, 0.15)";
    ctx.fillRect(0, 0, width, height);
    // Padding & Box Settings
    const margin = 140;
    const boxPadding = 70;
    const maxWidth = width - (margin * 2) - (boxPadding * 2);
    let fontSize = 65;
    ctx.font = `bold ${fontSize}px "Montserrat", "Arial"`;
    let lines = wrapText(ctx, text, maxWidth);
    // Dynamic Font Scaling
    while (lines.length * (fontSize * 1.4) > 600 && fontSize > 32) {
        fontSize -= 5;
        ctx.font = `bold ${fontSize}px "Montserrat", "Arial"`;
        lines = wrapText(ctx, text, maxWidth);
    }
    // Draw Frosted Glass Box
    ...
    ctx.shadowBlur = 40;
    ctx.fillStyle = "rgba(255, 255, 255, 0.88)"; // Glass look
    roundRect(ctx, boxX, boxY, boxWidth, boxHeight, 25);
    ctx.fill();
    ctx.shadowBlur = 0;
    // Final Text Draw (Main Quote)
    ctx.fillStyle = "#111111";
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    let textY = boxY + boxPadding + (lineHeight / 2);
    lines.forEach(line => {
        ctx.fillText(line.toUpperCase(), width / 2, textY);
        textY += lineHeight;
    });
    // ==========================================
    // ADDED: WATERMARK & BRANDING SECTION
    // ==========================================
    ...
    ctx.fillText("Daily Motivation", width / 2, boxY + boxHeight + 40);
...
// import fs from "fs";
// // Configuration for scalability
// const TRACKER_FILE = path.join(process.cwd(), "template_tracker.json");
// const TEMPLATE_DIR = path.join(process.cwd(), "public", "templates");
// /**
//  * Manages the image pool to ensure NO repeats until all images are used.
//  */
// import { motivationalQuotes } from "../data/quotes"; // Adjust path to your JSON/TS file
// /**
//  * Returns a random quote from your local database.
//  * No external API calls = 100% reliability.
//  */
...
//   try {
//     // 1. Pick a random index from your local array
//     const randomIndex = Math.floor(Math.random() * motivationalQuotes.length);
//     let quote = motivationalQuotes[randomIndex];
//     // 2. Clean up any existing quotes to avoid double-quotes
//     // e.g., ""Stay Hungry"" -> "Stay Hungry"
//     const cleanQuote = quote.replace(/^["']+|["']+$/g, '');
//     // 3. Return with standardized double quotes
//     return `"${cleanQuote}"`;
//   } catch (error) {
//     console.error("❌ Error picking local quote:", error);
//     // Hardcoded fallback just in case the file is missing
//     return `"The only way to do great work is to love what you do."`;
//   }
// }
// function getNextUniqueImage(): string {
//     const allImages = fs.readdirSync(TEMPLATE_DIR).filter(file => 
//         ['.jpg', '.jpeg', '.png'].includes(path.extname(file).toLowerCase())
//     );
//     let tracker = { used: [] as string[] };
//     if (fs.existsSync(TRACKER_FILE)) {
//         tracker = JSON.parse(fs.readFileSync(TRACKER_FILE, "utf-8"));
//     }
//     // Filter out images already used in this cycle
//     let available = allImages.filter(img => !tracker.used.includes(img));
//     // If all images have been used, reset the cycle
//     if (available.length === 0) {
//         console.log("♻️ All templates used once. Resetting cycle...");
//         tracker.used = [];
//         available = allImages;
//     }
...
//     return selected;
// }
// export async function createImageFromTemplate(text: string): Promise<Buffer> {
//     const imageName = getNextUniqueImage();
//     const width = 1080; // IG/FB 1:1 Square
//     const height = 1080;
//     const canvas = createCanvas(width, height);
//     const ctx = canvas.getContext("2d");
//     // 1. Draw Background (Cover logic)
//     const img = await loadImage(path.join(TEMPLATE_DIR, imageName));
...
//     // 2. Add Premium Gradient Overlay (Scrim)
//     // This makes text visible on any image (forest, mountain, or beach)
...
//     ctx.textAlign = "center";
//     ctx.textBaseline = "middle";
//     // Aesthetic: Use a larger font for shorter quotes
//     const fontSize = text.length < 50 ? 60 : 48;
//     ctx.font = `bold ${fontSize}px "Montserrat", "Arial"`;
//     // Wrap and Draw
...
//     // Add high-end typography shadow
//     ctx.shadowColor = "rgba(0, 0, 0, 0.5)";
//     ctx.shadowBlur = 20;
//     lines.forEach(line => {
//         ctx.fillText(line.toUpperCase(), width / 2, y);
//         y += lineHeight;
//     });
//     return canvas.toBuffer("image/png");
// }

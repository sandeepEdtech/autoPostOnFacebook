...
function getNextUniqueImage(): string {
    ...
    if (tracker.used.length > 10) tracker.used.shift(); // Keep memory to last 10
 ...
 */
async function fetchBackground(): Promise<any> {
    const useLocal = Math.random() < 0.4;

    if (useLocal && fs.existsSync(TEMPLATE_DIR)) {
        try {
            const imageName = getNextUniqueImage();
            return await loadImage(path.join(TEMPLATE_DIR, imageName));
        } catch (e) { console.log("Local fetch failed, switching to web..."); }
    }

    try {
        // Using Picsum for speed and reliability (No logo)
        return await loadImage(`https://picsum.photos/1080/1080?random=${Date.now()}`);
    } catch (error) {
        console.error("🌐 Web API failed. Using emergency local fallback...");
        const files = fs.readdirSync(TEMPLATE_DIR);
        return await loadImage(path.join(TEMPLATE_DIR, files[0]));
    }
}
...
function drawCoverImage(ctx: any, img: any, w: number, h: number) {
